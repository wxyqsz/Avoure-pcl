...
const Lifestyle = () => {
  ...
  useEffect(() => {
    async function fetchLifestyleArticles() {
      try {
        ...
        setError('Failed to load lifestyle articles. Please try again later.');
      } finally {
        setIsLoading(false);
      }
    }
  ...
  const featuredArticle = lifestyleArticles.length > 0 
    ? lifestyleArticles.find(article => article.featured) || lifestyleArticles[0]
    : null;
    
  // Get the remaining articles (excluding the featured one)
  const regularArticles = featuredArticle 
    ? lifestyleArticles.filter(article => article.id !== featuredArticle.id)
    : [];
      ...
      <section className="py-14 bg-black text-white mt-20">
        <div className="max-w-lg mx-auto px-4 text-center">
          ...
          <p className="text-gray-300 mb-6">
            Stay updated with the latest lifestyle trends and inspiration. Sign up for our newsletter.
          </p>
