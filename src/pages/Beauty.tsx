...
const Beauty = () => {
  ...
  useEffect(() => {
    async function fetchBeautyArticles() {
      try {
        // Fetch all beauty articles
        ...
        setBeautyArticles(data || []);
      } catch (err) {
        console.error('Error fetching beauty articles:', err);
        setError('Failed to load beauty articles. Please try again later.');
      } finally {
        setIsLoading(false);
      }
    }
  ...
  const featuredArticle = beautyArticles.length > 0 
    ? beautyArticles.find(article => article.featured) || beautyArticles[0]
    : null;
    
  // Get the remaining articles (excluding the featured one)
  const regularArticles = featuredArticle 
    ? beautyArticles.filter(article => article.id !== featuredArticle.id)
    : [];
      ...
      <section className="py-14 bg-black text-white mt-20">
        <div className="max-w-lg mx-auto px-4 text-center">
          ...
          <p className="text-gray-300 mb-6">
            Stay updated with the latest trends in beauty and skincare. Sign up for our newsletter.
          </p>
