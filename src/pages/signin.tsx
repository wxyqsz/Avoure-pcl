...
export default function AvoureSignIn() {
  ...
  // Improved admin authentication with better error handling
  ...
  // Improved subscriber authentication with better error handling
  const handleSubscriberAuth = async (normalizedEmail: string, password: string) => {
    const startTime = Date.now();
    addLog('Starting subscriber authentication');
    
    try {
      // Test database connection first with a more reliable approach
      addLog('Attempting to connect to Supabase');
      
      // Subscriber lookup with explicit timeout - increased to 20 seconds
      const dbQueryStart = Date.now();
      addLog('Querying subscriptions table');
      
      // Remove the race condition by handling the timeout differently
      ...
      // Check password - using simplified comparison
  ...
  // Improved submit handler - sequential auth for reliability
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Set timeout to prevent endless loading state - increased to 30 seconds for better reliability
    const timeout = setTimeout(() => {
      setIsSubmitting(false);
      setError('Request timed out. Please try again later.');
      addLog('Authentication timed out after 30 seconds');
    }, 30000);
    
    setTimeoutId(timeout);
    addLog('Set authentication timeout for 30 seconds');
    
    const normalizedEmail = normalizeEmail(email);
    addLog(`Normalized email: ${normalizedEmail}`);
    
    try {
      // Try admin authentication first (sequential approach is more reliable)
      ...
      if (adminResult.success) {
        ...
        // Redirect to admin dashboard with delay to ensure state updates complete
        addLog('Redirecting to admin dashboard with delay to ensure state updates complete');
        
        // First complete all state updates
        setIsSubmitting(false);
        
        // FIXED: Add delay before navigation to allow state updates to complete
        setTimeout(() => {
          addLog('Executing delayed navigation to admin dashboard');
          navigate('/admin');
        }, 300);
      ...
      if (subscriberResult.success) {
        ...
        // FIXED: Add delay before navigation to allow state updates to complete
        addLog('Redirecting subscriber to home page with delay');
        ...
        setTimeout(() => {
          addLog('Executing delayed navigation to home page');
          navigate('/');
        }, 300);
  ...
  // Check for existing session on mount with improved reliability
  useEffect(() => {
    const checkExistingSession = async () => {
      ...
      // Set timeout for the auth check (increased to 10 seconds for reliability)
      ...
      try {
        ...
        try {
          // Parse storedUser to check role directly without API call if possible
          const userData = JSON.parse(storedUser);
          const role = userData?.role;
          
          // FIXED: Check if we just completed auth to prevent navigation loop
          const adminAuthComplete = localStorage.getItem('adminAuthComplete');
          if (adminAuthComplete) {
            console.log('Just completed admin auth, skipping redirect');
          ...
          if (isAuthenticated) {
            ...
            // If checkAuth says not authenticated, clear any invalid stored data
          ...
          // Don't clear auth data on timeout, just continue to login screen
        }
      } catch (err) {
  ...
  const handleForgotPassword = async () => {
    ...
    // Set timeout to prevent endless loading state - increased to 20 seconds for reliability
    const timeout = setTimeout(() => {
      setIsSubmitting(false);
      setError('Request timed out. Please try again later.');
    ...
    try {
      // First check if it's an admin - with explicit timeout protection
      ...
      if (!adminError) {
        ...
        // FIXED: Add delay before navigation
        setTimeout(() => {
